...
var scaler = Vector2()
var wall_size: float = 0.8
var brick_size: float = 0.5
var paddle_size: float = 1.2
...
func ball_resize(size):
	if HUD.ball_size:
		scaler.x= 1.2
		scaler.y = 1.2
		$Sprite.scale = Vector2(0.3,0.3) * size
		$CollisionPolygon2D.scale = Vector2(0.3,0.3) * size
...
func _physics_process(_delta):
	...
	for body in bodies:
		...
		if body.name == "Walls":
			...
			ball_resize(wall_size)
			#$Sprite.visible = false
			#$Color.visible = true 
			#$Color. = Color8(224,49,49)
			$Sprite.modulate = Color8(51,154,240)
		if body.name == "Paddle":
			screen_shake(paddle_trauma)
			play_sound(effect_paddle)
			ball_resize(paddle_size)
			#$Sprite.visible = true
			#$Color.visible = false 
